...
CommonControls13_1_Details(TotalRuleCount, EnabledRulesCount) := Message if {
    DisabledRulesCount := TotalRuleCount - EnabledRulesCount
    Message := sprintf("Of the %d required rules, %d %s enabled and %d %s %s %s",
                       [TotalRuleCount,
                        EnabledRulesCount,
...
tests contains {
    ...
    "Prerequisites": ["policy/rule_system_defined_alerts"],
    "Criticality": "Shall",
    "ReportDetails": CommonControls13_1_Details(TotalRules, TotalEnabled),
    "ActualValue": {
...
if {
    SystemRules := input.policies[utils.TopLevelOU].rule_system_defined_alerts
    EnabledRules := [r.displayName | some r in SystemRules; r.state == "ACTIVE"]
    DisabledRules := [r.displayName | some r in SystemRules; r.state == "INACTIVE"]
    TotalEnabled := count(EnabledRules)
    TotalRules := count(SystemRules)
    Status := TotalRules == TotalEnabled
}
#--
