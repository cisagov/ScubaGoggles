...
class TestUtils:
    ...
    """Test suite for utility functions. This class contains tests for verifying
    the behavior of various utility functions in the project.
    """
    ...
    @pytest.mark.parametrize('base_filename, rel_segments',
        [('plain_file.txt',
         ...
         ['requirements.txt']),
         ('plain_file_2.txt',
         ['Testing', 'Python', 'testing_directory', 'secondary_directory',
          'plain_file_2.txt'])])
    ...
    @pytest.mark.parametrize('package, included',
        [('pyyaml', True),
         ('requests', True),
         ('tqdm', True),
         ('polars', False)])
    ...
    @pytest.mark.parametrize('user_input, default, expected',
        [('y', True, True),
         ...
         ('', False, False),
         ('red', True, ValueError)])
    def test_prompt_boolean(self, monkeypatch, user_input, default, expected):
        ...
        if isinstance(expected, type):
            with pytest.raises(expected):
                utils.prompt_boolean('', default=default)
        else:
            assert utils.prompt_boolean('', default=default) == expected

    @pytest.mark.parametrize('strval, expected, included',
        [('y', True, True),
         ('1', True, True),
         ('TRUE', True, True),
         ('no', False, True),
